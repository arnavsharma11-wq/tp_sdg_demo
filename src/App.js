  ...
  return (
    <div>
      <div style={{ position: "relative", width: "100%", aspectRatio: "16/9", borderRadius: 8, overflow: "hidden", background: `linear-gradient(180deg, #87CEEB ${40 - frame * 0.01}%, #6a6a6a 40%, #555 41%, #444 100%)`, border: `1px solid ${C.bdr}`, marginBottom: 8 }}>
        ...
        {showFaces && objects.filter(o => o.cls !== "vehicle").map(obj => (
          <div key={obj.id + "_face"} style={{ position: "absolute", left: `${obj.x + 1}%`, top: `${obj.y}%`, width: `${obj.w - 2}%`, height: `${obj.h * 0.4}%`, border: `2px solid ${facesOk ? C.green : C.red}`, borderRadius: 2, background: facesOk ? C.green + "20" : C.red + "10", transition: "left .03s" }}>
            <div style={{ position: "absolute", top: -10, left: 0, fontSize: 5, fontWeight: 700, color: facesOk ? C.green : C.red, background: "#000B", padding: "1px 2px", borderRadius: 2, whiteSpace: "nowrap" }}>{facesOk ? "âœ“" : "âš "}</div>
          </div>
        ))}
        {showFaces && <div style={{ position: "absolute", left: `${objects[1].x + 3}%`, top: `${objects[1].y + 6}%`, width: "6%", height: "2%", border: `1.5px solid ${facesOk ? C.green : C.red}`, borderRadius: 1 }}><div style={{ position: "absolute", top: -10, left: 0, fontSize: 5, fontWeight: 700, color: facesOk ? C.green : C.red, background: "#000B", padding: "1px 2px", borderRadius: 2, whiteSpace: "nowrap" }}>{facesOk ? "âœ“ PLATE" : "âš  PLATE"}</div></div>}
        <div style={{ position: "absolute", top: 6, left: 6, background: "#000A", padding: "3px 8px", borderRadius: 4, fontSize: 9, fontFamily: "monospace", color: "#0F0" }}>â— REC {fps}fps</div>
        <div style={{ position: "absolute", bottom: 6, left: 6, background: "#000A", padding: "3px 8px", borderRadius: 4, fontSize: 9, fontFamily: "monospace", color: "#0F0" }}>Frame {frame}/{totalFrames} Â· {sec}s</div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        <button onClick={() => setPlaying(!playing)} style={btn(C.cyan, true, { padding: "5px 12px", fontSize: 11 })}>{playing ? "â¸ Pause" : "â–¶ Play"}</button>
      ...
      <div style={{ fontSize: 8, color: C.txt, marginTop: 4 }}>â†‘ Temporal annotation timeline â€” colored bars show per-object tracking duration, white line = current frame</div>
    </div>
  );
}
...
const GEN_CHAT = [
  ...
  { role: "agent", text: "I hear you â€” that battery performance is way below what you should be getting. Let me help fix this.", sentiment: "empathetic", intent: "de_escalation", pii: false },
  ...
  { role: "agent", text: "I completely understand. Let me escalate this â€” I can process a full refund or send a replacement Pro unit with expedited shipping. Which would you prefer?", sentiment: "empathetic", intent: "offer_resolution", pii: false },
...
function ComplianceReport({ type, onClose }) {
  const data = {
    warehouse: { title: "Warehouse Safety â€” Image SDG", client: "Global Logistics Co.", id: "TP-SDG-WH-2026-0407", items: "423 annotated images", format: "COCO/VOC/YOLO", faces: "847 face instances replaced", plates: "N/A", diversity: "FID: 12.8", regulations: ["GDPR Art. 89", "CCPA/CPRA", "EU AI Act Art. 10", "ISO 27001", "SOC 2 Type II", "OSHA Compliance"] },
    driving: { title: "Autonomous Driving â€” Video SDG", client: "AV OEM [Confidential]", id: "TP-SDG-AV-2026-0407", items: "168 video clips (10s @ 30fps, 504K frames)", format: "MP4 H.264 + COCO-video JSON", faces: "847 face instances replaced", plates: "312 license plates masked", diversity: "FID: 12.4 Â· Temporal coherence: 0.94", regulations: ["GDPR Art. 89", "CCPA/CPRA", "EU AI Act Art. 10", "ISO 27001", "SOC 2 Type II", "NHTSA AV Guidelines"] },
    support: { title: "Customer Support â€” Text SDG", client: "FitPulse Inc.", id: "TP-SDG-CX-2026-0407", items: "1,723 conversations (10,683 turns)", format: "JSON, JSONL, CSV, Parquet", faces: "N/A", plates: "N/A", diversity: "Self-BLEU: 0.31 Â· Distinct-2: 0.87 Â· Entropy: 7.42", regulations: ["GDPR Art. 89", "CCPA/CPRA", "EU AI Act Art. 10", "SOC 2 Type II"] },
  }[type];
  ...
  return (
    <div style={{ ...cardS, borderColor: C.orange + "44" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <div style={{ fontSize: 14, fontWeight: 800, color: "#fff" }}>ðŸ“‹ TP.ai Compliance Report</div>
        <button onClick={onClose} style={{ background: "none", border: "none", color: C.txt, cursor: "pointer", fontSize: 14 }}>âœ•</button>
      </div>
      <div style={{ padding: 14, borderRadius: 8, background: C.bg, border: `1px solid ${C.bdr}`, fontFamily: "monospace", fontSize: 10, color: C.txt, lineHeight: 2 }}>
        ...
        {piiData}<br/>Re-identification risk: <strong style={{ color: C.green }}>0.003</strong> (target &lt;0.01) âœ“<br/>
        ...
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 6 }}>
          {data.regulations.map(r => <span key={r} style={{ padding: "2px 6px", borderRadius: 4, fontSize: 8, fontWeight: 700, background: C.green + "15", color: C.green }}>âœ“ {r}</span>)}
        </div>
        <div style={{ fontSize: 9, fontWeight: 700, color: C.cyan, letterSpacing: 1, marginTop: 6, marginBottom: 2 }}>5. DELIVERABLES</div>
        â˜‘ Synthetic dataset with annotations<br/>â˜‘ Prompt templates & generation configs<br/>â˜‘ De-identification audit log<br/>â˜‘ License & ToS snapshots with dates<br/>â˜‘ This compliance report (PDF)<br/>
        ...
        <div style={{ borderTop: `1px solid ${C.bdr}`, paddingTop: 6, marginTop: 8, textAlign: "center", fontSize: 8 }}><strong style={{ color: C.hi }}>TP.ai Data Services</strong> Â· TP Â· Confidential</div>
      </div>
    </div>
  );
}
...
function PipelineDemo({ type, onBack }) {
  ...
  const conf = {
    warehouse: { label: "Warehouse Safety Â· Image SDG", color: C.amber, icon: "ðŸ­", prompts: ["Workers + Forklift + PPE Violations", "Night Shift + Spilled Cargo", "High-Traffic Stress Test", "Partial Occlusion Edge Case"], genLabel: "images", vol: 500, piiLabel: "3 faces detected", deIdLabel: "Faces replaced (synthetic, not blur)" },
    driving: { label: "Self-Driving Â· Video SDG", color: C.cyan, icon: "ðŸš—", prompts: ["Pedestrian Crossing + Red Light", "Heavy Rain + Wet Road", "Night + Headlight Glare", "Construction Zone + Lane Shift"], genLabel: "video clips", vol: 200, piiLabel: "3 faces + 1 license plate", deIdLabel: "Faces replaced + plates masked across all frames" },
    support: { label: "Angry Customer Support Â· Text SDG", color: C.red, icon: "ðŸ’¬", prompts: ["Battery Rage â†’ Refund Demand", "Sleep Tracking Sarcasm", "Skin Rash + Legal Threat", "Subscription Price Hike Fury"], genLabel: "conversations", vol: 2000, piiLabel: "4,291 PII entities (names, emails, addresses, order IDs)", deIdLabel: "Entity-level masking applied" },
  }[type];
  ...
  return (
    <div style={{ background: C.bg, color: C.txt, fontFamily: "'TP Sans', 'DM Sans', sans-serif", minHeight: "calc(100vh - 112px)" }}>
      <div style={{ padding: "8px 24px", display: "flex", alignItems: "center", gap: 12, borderBottom: `1px solid ${C.bdr}` }}>
        <button style={btn(C.txt, true, { padding: "4px 10px", fontSize: 10 })} onClick={onBack}>â† All Journeys</button>
          ...
          <div style={{ display: "flex", gap: 16 }}>
            <div style={{ flex: "0 0 340px" }}>
              <div style={cardS}>
                ...
                <div style={{ fontSize: 11, fontWeight: 700, color: C.hi, marginBottom: 6, display: "flex", alignItems: "center" }}>ðŸ“ Seed Data <HT /> <span style={{ fontSize: 9, color: C.amber, marginLeft: 4 }}>Domain Experts</span></div>
                ...
                <button style={btn(C.accent, false, { width: "100%" })} onClick={() => setStage(1)}>Config Complete â†’ Generate</button>
              </div>
            </div>
            <div style={{ flex: 1 }}>
              <div style={cardS}>
                <div style={{ fontSize: 12, fontWeight: 700, color: C.hi, marginBottom: 4, display: "flex", alignItems: "center" }}>âœï¸ Prompt Templates <HT /> <span style={{ fontSize: 9, color: C.amber, marginLeft: 4 }}>Prompt Engineers</span></div>
          ...
          <div style={{ display: "flex", gap: 16 }}>
            <div style={{ flex: "0 0 280px" }}>
              <div style={cardS}>
                ...
                {!genOn && !genDone && <button style={btn("#8B5CF6", false, { width: "100%" })} onClick={runGen}>ðŸ§  Generate {conf.genLabel}</button>}
                ...
                {genDone && <div><div style={{ fontSize: 12, color: C.green, fontWeight: 600, marginBottom: 6 }}>âœ“ {conf.vol} {conf.genLabel} generated</div><div style={{ padding: 5, borderRadius: 5, background: C.amber + "10", border: `1px solid ${C.amber}33`, marginBottom: 8, fontSize: 9, color: C.amber, display: "flex", alignItems: "center", gap: 3 }}><HT s={6}/> QA validated</div><button style={btn(tc, false, { width: "100%" })} onClick={() => setStage(2)}>Critique â†’</button></div>}
              </div>
            </div>
            <div style={{ flex: 1 }}>
              <div style={cardS}>
          ...
          <div style={{ display: "flex", gap: 16 }}>
            <div style={{ flex: "0 0 280px" }}>
              <div style={cardS}>
                ...
                <div style={{ fontSize: 10, color: C.red, fontWeight: 700, marginBottom: 8 }}>ðŸ”„ ITERATIVE LOOP</div>
                ...
                {Object.values(approvals).filter(v => v).length >= 4 && <button style={btn(tc, false, { width: "100%" })} onClick={() => setStage(3)}>Quality Gate â†’</button>}
              </div>
            </div>
            <div style={{ flex: 1 }}>
              <div style={cardS}>
                    ...
                    <div key={i} style={{ borderRadius: 8, overflow: "hidden", background: C.bg, border: `1px solid ${approvals[i] === "ok" ? C.green + "55" : approvals[i] === "no" ? C.red + "55" : C.bdr}` }}>
                      ...
                      <div style={{ padding: 10 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                          <span style={{ fontSize: 10, fontWeight: 700, color: C.hi }}>#{i+1} {p.split("+")[0].split("â†’")[0].trim()}</span>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 6 }}>
                          ...
                          {[1,2,3,4,5].map(s => <span key={s} onClick={() => setRatings(r => ({ ...r, [i]: s }))} style={{ fontSize: 14, cursor: "pointer", filter: (ratings[i]||0) >= s ? "none" : "grayscale(1) opacity(0.25)" }}>â­</span>)}
                        </div>
                        <div style={{ display: "flex", gap: 4 }}>
                          <button onClick={() => setApprovals(a => ({ ...a, [i]: "ok" }))} style={btn(C.green, approvals[i] !== "ok", { padding: "4px 10px", fontSize: 9, flex: 1 })}>âœ“ Approve</button>
                          <button onClick={() => setApprovals(a => ({ ...a, [i]: "no" }))} style={btn(C.red, approvals[i] !== "no", { padding: "4px 10px", fontSize: 9, flex: 1 })}>âœ— Reject</button>
                        </div>
                      </div>
                    </div>
                    );
          ...
          <div style={{ display: "flex", gap: 16 }}>
            <div style={{ flex: "0 0 280px" }}>
              <div style={cardS}>
                ...
                {filterDone && <div style={{ marginTop: 8 }}><div style={{ fontSize: 11, color: C.green, fontWeight: 600, marginBottom: 6 }}>âœ“ Curation complete</div><button style={btn(tc, false, { width: "100%" })} onClick={() => setStage(4)}>Compliance â†’</button></div>}
              </div>
            </div>
            <div style={{ flex: 1 }}>
              <div style={cardS}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                  {[{ l: "Input", n: conf.vol }, { l: "Quality", n: Math.round(conf.vol * 0.95) }, { l: "Dedup", n: Math.round(conf.vol * 0.9) }, { l: "Safety", n: Math.round(conf.vol * 0.87), h: 1 }, { l: "Routed", n: Math.round(conf.vol * 0.84), h: 1 }, { l: "Final", n: Math.round(conf.vol * 0.84) }].map((f, i) => (
                    <div key={i} style={{ padding: 12, borderRadius: 8, background: C.bg, border: `1px solid ${filterDone ? C.green + "33" : C.bdr}`, opacity: filterDone ? 1 : 0.3, transition: "all .5s", transitionDelay: `${i * 150}ms`, textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 9, color: C.txt }}>{f.l}{f.h ? " ðŸ‘¥" : ""}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div style={{ display: "flex", gap: 16 }}>
            <div style={{ flex: "0 0 280px" }}>
              <div style={cardS}>
                ...
                {!piiDone && <button style={btn(C.orange, false, { width: "100%" })} onClick={() => setTimeout(() => setPiiDone(true), 2000)}>ðŸ” Detect PII</button>}
                {piiDone && !deIdDone && <div><div style={{ fontSize: 10, color: C.green, fontWeight: 600, marginBottom: 4 }}>âœ“ {conf.piiLabel}</div><button style={btn(C.orange, false, { width: "100%" })} onClick={() => setDeIdDone(true)}>ðŸŽ­ De-identify</button></div>}
                {deIdDone && !privacyOk && (
                  <div style={{ padding: 10, borderRadius: 8, background: "#1a1500", border: `2px solid ${C.amber}`, marginTop: 4 }}>
                    ...
                    <div style={{ display: "flex", gap: 5 }}>
                      <button style={btn(C.green, false, { padding: "5px 10px", fontSize: 10 })} onClick={() => setPrivacyOk(true)}>âœ“ Approve</button>
                      <button style={btn(C.red, true, { padding: "5px 10px", fontSize: 10 })} onClick={() => { setDeIdDone(false); setPiiDone(false); }}>âœ— Revise</button>
                    </div>
                  </div>
                )}
                {privacyOk && <div style={{ marginTop: 8 }}><div style={{ fontSize: 11, color: C.green, fontWeight: 600, marginBottom: 6 }}>âœ“ Privacy Audit Passed</div><button style={btn(tc, false, { width: "100%" })} onClick={() => setStage(5)}>Annotate â†’</button></div>}
              </div>
            </div>
            <div style={{ flex: 1 }}>
              <div style={cardS}>
                ...
                <div style={{ fontSize: 9, color: C.txt, marginTop: 6 }}>{!piiDone ? "Run detection to identify PII" : !deIdDone ? "PII detected. Click de-identify." : "âœ“ All PII de-identified. Re-identification risk: 0.003"}</div>
              </div>
            </div>
          </div>
        )}
          ...
          <div style={{ display: "flex", gap: 16 }}>
            <div style={{ flex: "0 0 300px" }}>
              <div style={cardS}>
                ...
                {!labelsOk && <div style={{ padding: 8, borderRadius: 6, background: C.amber + "10", border: `1px solid ${C.amber}33`, marginBottom: 8 }}><div style={{ fontSize: 9, fontWeight: 700, color: C.amber, display: "flex", alignItems: "center", gap: 3, marginBottom: 3 }}><HT s={6}/> Labelers & Data Librarians</div><button style={btn(C.green, false, { padding: "6px 12px", fontSize: 10 })} onClick={() => setLabelsOk(true)}>âœ“ Approve Annotations</button></div>}
                {labelsOk && !packed && <button style={btn(C.green, false, { width: "100%" })} onClick={() => setTimeout(() => setPacked(true), 1000)}>ðŸ“¦ Package</button>}
                {packed && (
                  <div>
                    <div style={{ fontSize: 14, color: C.green, fontWeight: 700, marginBottom: 6 }}>ðŸŽ‰ Complete!</div>
                    <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                      <button style={btn(C.orange, false, { flex: 1, fontSize: 10 })} onClick={() => setShowReport(true)}>ðŸ“‹ Report</button>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 3, marginBottom: 16 }}>
      {stageLabels.map((l, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center" }}>
          <div onClick={() => i <= maxStage && advance(i)} title={l} style={{ width: 44, height: 44, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, cursor: i <= maxStage ? "pointer" : "default", background: stage === i ? stageColors[i] + "22" : i < stage ? C.green + "18" : C.card, border: `2px solid ${stage === i ? stageColors[i] : i < stage ? C.green : C.bdr}`, color: stage === i ? "#fff" : i < stage ? C.green : C.txt }}>
            {i < stage ? "âœ“" : i + 1}
          </div>
          {i < stageLabels.length - 1 && <div style={{ width: 8, height: 2, background: i < stage ? C.green + "44" : C.bdr }} />}
        </div>
      ))}
      <span style={{ marginLeft: 8, fontSize: 18, color: C.txt, fontWeight: 700, letterSpacing: "0.08em" }}>{label}</span>
    </div>
  );
}
...
function ChipGroup({ label, options, value, onChange, color }) {
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ fontSize: 18, fontWeight: 700, color: C.hi, marginBottom: 5 }}>{label}</div>
      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
        {options.map(o => (
          <button key={o} onClick={() => onChange(o)} style={{ padding: "4px 10px", borderRadius: 6, fontSize: 18, fontWeight: 600, border: `1px solid ${value === o ? color : C.bdr}`, background: value === o ? color + "20" : C.bg, color: value === o ? color : C.txt, cursor: "pointer", fontFamily: "'TP Sans','DM Sans',sans-serif" }}>{o}</button>
        ))}
      </div>
    </div>
  );
...
const HDG_AUTHORED = {
  "Customer Support": [
    ...
    { role: "agent", text: "I see it â€” there was a carrier pickup delay. I'm escalating to priority shipping now.", sentiment: "proactive", intent: "resolve" },
  ...
  "Healthcare Intake": [
    { role: "customer", text: "I've had chest pressure since this morning â€” it radiates to my left arm.", sentiment: "worried", intent: "symptom_report" },
  ...
  "Legal Q&A": [
    ...
    { role: "customer", text: "Yes â€” three certified letters sent over six months.", sentiment: "certain", intent: "confirm" },
  ...
  "Technical Support": [
    ...
    { role: "customer", text: "No â€” I was using text/plain. Could that affect authentication?", sentiment: "curious", intent: "clarify" },
        ...
        <div style={{ display: "flex", gap: 16 }}>
          <div style={{ flex: "0 0 320px" }}>
            <div style={cardS}>
              ...
              <button style={btn(C.accent, false, { width: "100%", marginTop: 8 })} onClick={() => advance(1)}>Commission Task â†’</button>
            </div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={cardS}>
              ...
              <div style={{ padding: 14, borderRadius: 8, background: C.bg, border: `1px solid ${C.bdr}`, fontFamily: "monospace", fontSize: 20, color: C.txt, lineHeight: 2 }}>
                ...
                {[["Domain", domain], ["Persona", persona], ["Difficulty", difficulty], ["Target turns", difficulty === "Baseline" ? "4â€“6" : difficulty === "Moderate" ? "6â€“8" : "8â€“12"], ["Language", "English (en-US)"], ["Format", "Chat conversation"]].map(([k, v]) => (
                  <div key={k}><strong style={{ color: C.hi }}>{k}:</strong> {v}<br/></div>
                ))}
                <div style={{ marginTop: 6 }}><strong style={{ color: C.hi }}>Status:</strong> <span style={{ color: C.amber }}>â³ Pending assignment</span></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          <div style={{ flex: "0 0 280px" }}>
            <div style={cardS}>
              ...
              {!authoring && !authorDone && <button style={btn("#8B5CF6", false, { width: "100%" })} onClick={runAuthor}>âœï¸ Simulate Authoring</button>}
              ...
              {authorDone && <div><div style={{ fontSize: 22, color: C.green, fontWeight: 600, marginBottom: 6 }}>âœ“ Submission received</div><div style={{ padding: 8, borderRadius: 6, background: C.green + "10", border: `1px solid ${C.green}22`, fontSize: 18, color: C.green, marginBottom: 10 }}>Contributor CTR-4471 Â· {chat.length} turns Â· 0m ago</div><button style={btn(C.accent, false, { width: "100%" })} onClick={() => advance(2)}>Send to QA â†’</button></div>}
            </div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={cardS}>
              <div style={{ fontSize: 22, fontWeight: 700, color: C.hi, marginBottom: 8 }}>Authored Output â€” {domain}</div>
          ...
          <div style={{ flex: "0 0 280px" }}>
            <div style={cardS}>
              ...
              <div style={{ marginTop: 10 }}>
                {flagCount > 0 && <div style={{ fontSize: 18, color: C.red, marginBottom: 6 }}>âš  {flagCount} flag(s) â€” resolve before approving</div>}
                {avgRating && flagCount === 0 && !qaApproved && (
                  <div style={{ display: "flex", gap: 5 }}>
                    <button style={btn(C.green, false, { flex: 1, padding: "6px 10px", fontSize: 20 })} onClick={() => setQaApproved(true)}>âœ“ Approve</button>
                    <button style={btn(C.red, true, { flex: 1, padding: "6px 10px", fontSize: 20 })} onClick={() => { setStage(1); setAuthorDone(false); setAuthorProg(0); setRatings({}); setFlags({}); }}>âœ— Reject</button>
                  </div>
                )}
                {!avgRating && !flagCount && <div style={{ fontSize: 18, color: C.txt }}>Rate all turns to enable approval</div>}
                {qaApproved && <div><div style={{ fontSize: 22, color: C.green, fontWeight: 600, marginBottom: 6 }}>âœ“ QA Passed Â· Avg {avgRating}/5.0</div><button style={btn(C.cyan, false, { width: "100%" })} onClick={() => advance(3)}>Label â†’</button></div>}
              </div>
            </div>
          </div>
          <div style={{ flex: 1 }}>
              ...
              {chat.map((turn, i) => (
                <div key={i} style={{ padding: 10, borderRadius: 8, background: C.bg, border: `1px solid ${ratings[i] ? C.bdr : C.bdr}`, marginBottom: 6 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                    <span style={{ fontSize: 18, fontWeight: 700, color: turn.role === "customer" ? C.red : C.green }}>{turn.role === "customer" ? "ðŸ‘¤ User" : "ðŸŽ§ Agent"} Â· Turn {i + 1}</span>
                    <div style={{ display: "flex", gap: 2 }}>{[1,2,3,4,5].map(s => <span key={s} onClick={() => setRatings(r => ({ ...r, [i]: s }))} style={{ fontSize: 28, cursor: "pointer", filter: (ratings[i]||0) >= s ? "none" : "grayscale(1) opacity(0.18)" }}>â­</span>)}</div>
                  </div>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          <div style={{ flex: "0 0 300px" }}>
            <div style={cardS}>
              ...
              {appliedIntent.length > 0 && appliedDiff && !labelsDone && <button style={btn(C.cyan, false, { width: "100%" })} onClick={() => setLabelsDone(true)}>âœ“ Confirm Labels</button>}
              {labelsDone && <div><div style={{ fontSize: 22, color: C.green, fontWeight: 600, marginBottom: 6 }}>âœ“ Labels confirmed</div><button style={btn(C.green, false, { width: "100%" })} onClick={() => advance(4)}>Package â†’</button></div>}
            </div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={cardS}>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          <div style={{ flex: "0 0 300px" }}>
            <div style={cardS}>
              ...
              {!packed && <button style={btn(C.green, false, { width: "100%" })} onClick={() => setTimeout(() => setPacked(true), 1000)}>ðŸ“¦ Package Record</button>}
              {packed && <div><div style={{ fontSize: 24, color: C.green, fontWeight: 700, marginBottom: 10 }}>ðŸŽ‰ Delivered to corpus</div><button style={btn(C.accent, true, { width: "100%", fontSize: 20 })} onClick={reset}>New Task</button></div>}
            </div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={cardS}>
              ...
              <div style={{ padding: 14, borderRadius: 8, background: C.bg, border: `1px solid ${C.bdr}`, fontFamily: "monospace", fontSize: 20, color: C.txt, lineHeight: 2 }}>
                ...
                {[["Domain", domain], ["Persona", persona], ["Difficulty", appliedDiff || difficulty], ["Contributor", "CTR-4471 (anonymized)"], ["Submitted", new Date().toISOString()], ["QA Score", avgRating + " / 5.0 âœ“"], ["Turns", chat.length], ["Intent Tags", appliedIntent.join(", ") || "â€”"], ["Sentiment Tags", appliedSentiment.join(", ") || "â€”"], ["Format", "JSONL Â· UTF-8"], ["Pipeline", "TP.ai HDG v1.0"], ["Status", packed ? "âœ“ Delivered" : "â³ Pending"]].map(([k, v], i) => (
...
const HDC_CLIPS = [
  ...
  { id: "CLR-002", city: "Mumbai",    gender: "M", age: 38, device: "iPhone",  dur: 4.1,  snr: 5,  asr: "[inaudible â€” heavy background noise throughout]",                corrected: null },
  ...
  { id: "CLR-004", city: "Hyderabad", gender: "M", age: 45, device: "Android", dur: 2.8,  snr: 19, asr: "[clip truncated â€” too short]",                                    corrected: null },
  ...
  { id: "CLR-007", city: "Kolkata",   gender: "F", age: 52, device: "Android", dur: 6.7,  snr: 7,  asr: "[background music detected â€” music-speech ratio 0.78]",          corrected: null },
...
function HumanDataCollectDemo() {
  ...
  const [duration, setDuration] = useState("5â€“15s");
        ...
        <div style={{ display: "flex", gap: 16 }}>
          <div style={{ flex: "0 0 320px" }}>
            <div style={cardS}>
              ...
              <ChipGroup label="Clip Duration" options={["3â€“10s","5â€“15s","10â€“30s","30â€“60s"]} value={duration} onChange={setDuration} color={C.cyan} />
              <button style={btn(C.cyan, false, { width: "100%", marginTop: 8 })} onClick={() => advance(1)}>Launch Campaign â†’</button>
            </div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={cardS}>
              ...
              <div style={{ padding: 14, borderRadius: 8, background: C.bg, border: `1px solid ${C.bdr}`, fontFamily: "monospace", fontSize: 20, color: C.txt, lineHeight: 2 }}>
                ...
                {[["Type", dataType], ["Geography", geography], ["Target", target], ["Duration", duration], ["Consent", "âœ“ GDPR + Regional"], ["Anonymization", "âœ“ Voice-print scrub on ingest"], ["Status", "â³ Ready to launch"]].map(([k, v]) => (
        ...
        <div style={{ display: "flex", gap: 16 }}>
          <div style={{ flex: "0 0 280px" }}>
            <div style={cardS}>
              ...
              {!capturing && !capDone && <button style={btn("#8B5CF6", false, { width: "100%" })} onClick={runCapture}>ðŸ“¡ Simulate Ingestion</button>}
              ...
              {capDone && <div><div style={{ fontSize: 22, color: C.green, fontWeight: 600, marginBottom: 6 }}>âœ“ {HDC_CLIPS.length} clips ingested</div><div style={{ padding: 8, borderRadius: 6, background: C.green + "10", border: `1px solid ${C.green}22`, fontSize: 18, color: C.green, marginBottom: 10 }}>Voice-print scrub complete Â· PII metadata stripped</div><button style={btn(C.amber, false, { width: "100%" })} onClick={() => advance(2)}>Apply QA Filters â†’</button></div>}
            </div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={cardS}>
            ...
            <div style={cardS}>
              ...
              <div style={{ padding: 10, borderRadius: 6, background: C.bg, border: `1px solid ${C.bdr}`, marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 24, marginBottom: 6 }}>
                  <span style={{ color: C.green, fontWeight: 700 }}>âœ“ Pass: {passCount}</span>
                  <span style={{ color: C.red, fontWeight: 700 }}>âœ— Fail: {HDC_CLIPS.length - passCount}</span>
                </div>
              ...
              <button style={btn(C.orange, false, { width: "100%" })} onClick={() => advance(3)}>Apply â†’ Annotate</button>
            </div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={cardS}>
                ...
                {HDC_CLIPS.map(clip => {
                  ...
                  return (
                    <div key={clip.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 10px", borderRadius: 6, background: C.bg, border: `1px solid ${pass ? C.green + "44" : C.red + "33"}`, fontSize: 18, transition: "border-color .2s" }}>
                      <span style={{ color: pass ? C.green : C.red, fontSize: 24, width: 14 }}>{pass ? "âœ“" : "âœ—"}</span>
                      <span style={{ fontFamily: "monospace", color: C.hi, minWidth: 62 }}>{clip.id}</span>
                      <span style={{ color: C.txt }}>{clip.city} Â· {clip.gender}/{clip.age} Â· {clip.device}</span>
                      <span style={{ color: C.txt }}>{clip.dur}s Â· {clip.snr}dB</span>
          ...
          <div style={{ flex: "0 0 280px" }}>
            <div style={cardS}>
              ...
              <div style={{ padding: 8, borderRadius: 6, background: C.cyan + "10", border: `1px solid ${C.cyan}22`, fontSize: 18, color: C.cyan, marginBottom: 10 }}>
                {passCount} clips passed Â· showing {annotateSet.length} for review
              </div>
              <div style={{ padding: 8, borderRadius: 6, background: C.bg, border: `1px solid ${C.bdr}`, fontSize: 18, color: C.txt, lineHeight: 1.8, marginBottom: 10 }}>
                <div><strong style={{ color: C.green }}>âœ“ Approved:</strong> {approvedCount}</div>
                <div><strong style={{ color: C.amber }}>âœŽ Corrected:</strong> {correctedCount}</div>
                <div><strong style={{ color: C.txt }}>â³ Pending:</strong> {annotateSet.length - approvedCount - correctedCount}</div>
              </div>
              {annotationsDone && <button style={btn(C.green, false, { width: "100%" })} onClick={() => advance(4)}>Package Dataset â†’</button>}
            </div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={cardS}>
                  ...
                  <div key={clip.id} style={{ padding: 10, borderRadius: 8, background: C.bg, border: `1px solid ${ann ? (ann === "corrected" ? C.amber + "55" : C.green + "44") : C.bdr}`, marginBottom: 8 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                      <div>
                        ...
                        <span style={{ fontSize: 16, color: C.txt, marginLeft: 8 }}>{clip.city} Â· {clip.gender}/{clip.age} Â· {clip.dur}s Â· SNR {clip.snr}dB</span>
                      </div>
                      {!ann && (
                        <div style={{ display: "flex", gap: 4 }}>
                          <button onClick={() => setAnnotations(a => ({ ...a, [clip.id]: "approved" }))} style={{ padding: "3px 8px", borderRadius: 4, fontSize: 18, fontWeight: 700, background: C.green + "18", border: `1px solid ${C.green}33`, color: C.green, cursor: "pointer", fontFamily: "inherit" }}>âœ“ Approve</button>
                          {hasError && <button onClick={() => setAnnotations(a => ({ ...a, [clip.id]: "corrected" }))} style={{ padding: "3px 8px", borderRadius: 4, fontSize: 18, fontWeight: 700, background: C.amber + "18", border: `1px solid ${C.amber}33`, color: C.amber, cursor: "pointer", fontFamily: "inherit" }}>âœŽ Correct</button>}
                        </div>
                      )}
                      {ann && <span style={{ fontSize: 18, fontWeight: 700, color: ann === "corrected" ? C.amber : C.green }}>{ann === "corrected" ? "âœŽ Corrected" : "âœ“ Approved"}</span>}
                    </div>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          <div style={{ flex: "0 0 300px" }}>
            <div style={cardS}>
              ...
              {!packed && <button style={btn(C.green, false, { width: "100%" })} onClick={() => setTimeout(() => setPacked(true), 1200)}>ðŸ“¦ Package</button>}
              {packed && (
                <div>
                  <div style={{ fontSize: 24, color: C.green, fontWeight: 700, marginBottom: 10 }}>ðŸŽ‰ Dataset ready</div>
          ...
          <div style={{ flex: 1 }}>
            <div style={cardS}>
              ...
              <div style={{ padding: 14, borderRadius: 8, background: C.bg, border: `1px solid ${C.bdr}`, fontFamily: "monospace", fontSize: 20, color: C.txt, lineHeight: 2 }}>
                ...
                {[["Campaign", `${dataType} Â· ${geography}`], ["Collected", new Date().toISOString().split("T")[0]], ["Pipeline", "TP.ai HDC v1.0"], ["Ingested", HDC_CLIPS.length + " clips"], ["QA filters", `SNR â‰¥ ${minSNR}dB Â· Duration â‰¥ ${minDur}s`], ["Passed QA", passCount + " clips"], ["Reviewed", Object.keys(annotations).length + " transcripts"], ["Anonymization", "âœ“ Voice-print scrub Â· Metadata stripped"], ["Consent", "âœ“ All participants consented"], ["Format", "WAV 16kHz mono + JSONL"], ["Status", packed ? "âœ“ Ready for delivery" : "â³ Packaging..."]].map(([k, v]) => (
    ...
    <div style={{ marginTop: 40, paddingTop: 28, borderTop: `1px solid ${C.bdr}` }}>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 260px", padding: "14px 18px", borderRadius: 8, background: C.card, border: `1px solid ${C.bdr}` }}>
          ...
          <div style={{ fontSize: 12, color: C.txt, lineHeight: 1.6 }}>Prescriptive â€” you define what the data should be, humans create it</div>
        </div>
        <div style={{ flex: "1 1 260px", padding: "14px 18px", borderRadius: 8, background: C.card, border: `1px solid ${C.bdr}` }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: C.cyan, marginBottom: 6 }}>Collection</div>
          <div style={{ fontSize: 12, color: C.txt, lineHeight: 1.6 }}>Descriptive â€” the world produces signals, humans capture and label them</div>
        </div>
      </div>
    </div>
  );
...
const HDG_STAGES = [
  ...
  { title: "Data Authoring", desc: "Contributors produce original content â€” conversations, prompts, responses, narratives, or code." },
...
const HDG_EXAMPLE_CHAT = [
  ...
  { role: "agent", text: "I'm sorry about that â€” let me take a look.", sentiment: "empathetic", intent: "acknowledge" },
    ...
    <div style={{ padding: "32px 40px 48px", maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 12, color: C.txt, margin: 0, maxWidth: 680, lineHeight: 1.65 }}>
          Humans generate data intentionally according to clear task briefs â€” filling gaps that real-world data cannot cover such as rare scenarios, sensitive interactions, adversarial prompts, multilingual edge cases, or policy-driven content.
        </p>
      </div>
        ...
        <div style={{ flex: "1 1 320px", minWidth: 280 }}>
          <div style={{ ...cardS, borderColor: C.accent + "33" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: C.accent, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10 }}>
              Example â€” Synthetic Customer Support Conversation
            </div>
  ...
  return (
    <div style={{ padding: "32px 40px 48px", maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 12, color: C.txt, margin: 0, maxWidth: 680, lineHeight: 1.65 }}>
          Human data collection focuses on capturing authentic behavior â€” speech, images, video, or interactions â€” through participant consent, controlled collection environments, and rigorous processing pipelines.
        </p>
      </div>
        ...
        <div style={{ flex: "1 1 320px", minWidth: 280 }}>
          <div style={{ ...cardS, borderColor: C.cyan + "33" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: C.cyan, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 14 }}>
              Example â€” Voice Data Collection for Speech Recognition
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
              {HDC_CHECKLIST.map((item, i) => (
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, fontSize: 12, color: C.hi, lineHeight: 1.5 }}>
                  <span style={{ color: C.cyan, fontSize: 11, marginTop: 1, flexShrink: 0, fontWeight: 700 }}>âœ“</span>
                  {item}
                </div>
              ))}
            </div>
  ...
  return (
    <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "calc(100vh - 112px)", padding: 32 }}>
      ...
      <div style={{ position: "relative", zIndex: 2, display: "flex", gap: 18, flexWrap: "wrap", justifyContent: "center", marginBottom: 48 }}>
        {[
          { k: "warehouse", icon: "ðŸ­", title: "Warehouse Safety", sub: "Image SDG", desc: "Generate synthetic warehouse images with workers, forklifts, safety violations, and edge cases from a single seed photo.", color: C.amber, brief: "Logistics client Â· 500 annotated images" },
          { k: "driving", icon: "ðŸš—", title: "Self-Driving Car", sub: "Video SDG", desc: "Generate dashcam video clips with pedestrians, rain, night driving, and construction zones for perception training.", color: C.cyan, brief: "AV OEM Â· 200 video clips Â· 30fps" },
          { k: "support", icon: "ðŸ’¬", title: "Angry Customer Support", sub: "Text SDG", desc: "Generate customer support conversations with angry, sarcastic, and threatening customers for support AI training.", color: C.red, brief: "FitPulse wearable Â· 2,000 conversations" },
        ].map(j => (
          <div key={j.k}
      ...
      <div style={{ position: "relative", zIndex: 2, display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
        ...
        <div style={{ padding: "8px 14px", borderRadius: 6, background: C.green + "10", border: `1px solid ${C.green}22`, fontSize: 10, color: C.green, fontWeight: 600 }}>ðŸ”’ GDPR Â· HIPAA Â· EU AI Act Â· SOC 2</div>
        <div style={{ padding: "8px 14px", borderRadius: 6, background: C.accent + "10", border: `1px solid ${C.accent}22`, fontSize: 10, color: C.accent, fontWeight: 600 }}>ðŸ“‹ Full compliance report per delivery</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.txt, fontFamily: "'TP Sans', 'DM Sans', sans-serif" }}>
      {/* Blurred GIF strip â€” fixed at bottom across all tabs */}
